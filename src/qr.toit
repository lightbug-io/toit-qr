...
Currently supports version 3 (29x29 modules), ECC level L, and byte mode.
  Produces a matrix (a list of lists of 0/1) that can be rendered to any
  display format.

# Examples
```
qr := QrCode.encode "https://example.com/foo"
matrix := qr.matrix  // List of lists of int (0 or 1).
size := qr.size      // 29.
```

To convert the matrix to a 1-bit-per-pixel bitmap (for example for e-ink):
```
...
Uses the generator polynomial 0x11D (x^8 + x^4 + x^3 + x^2 + 1).
...
/**
QR code encoder for version 3 (29x29 modules), ECC level L, byte mode.

Use $encode to build a QR code from a string, or $encode-bytes to build one
  from raw bytes. The result is available as $matrix and $size.
*/
class QrCode:
  /** The QR version produced by this encoder. */
  static VERSION  ::= 3
  /** The side length in modules of the QR codes produced by this encoder. */
  static SIZE     ::= 29
  /** The maximum number of data bytes that can be encoded (byte mode, ECC level L). */
  static MAX-DATA-BYTES ::= 42
  static DATA-CODEWORDS_  ::= 55
  static EC-CODEWORDS_    ::= 15
  static TOTAL-CODEWORDS_ ::= 70

  /** This QR code as a 29x29 list of lists, each module 0 (white) or 1 (black). */
  matrix/List
  /** The side length of this QR code in modules. */
  size/int

  constructor.internal_ .matrix .size:

  /**
  Encodes the given UTF-8 $text as a QR code.

  The $text must fit in version 3 byte mode (at most $MAX-DATA-BYTES bytes
    for ECC level L).
  */
  ...
  /**
  Encodes the given raw $data as a QR code.

  # Errors
  It is an error if $data is larger than $MAX-DATA-BYTES bytes (the version 3
  ...
  */
  ...
  /** Encodes $data into QR code data codewords (byte mode, ECC level L). */
  ...
  /** Generates the Reed-Solomon error-correction codewords for $data. */
  ...
  /**
  Computes the Reed-Solomon generator polynomial with $num-ec coefficients.

  Returns the coefficients in standard order: leading coefficient first.
  */
  static rs-generator-poly_ num-ec/int -> ByteArray:
    ...
    // Reverse to standard order: leading coefficient (x^n) first, constant
    // term last. The construction above builds coefficients with the constant
    // term first; RS division expects the leading term first.
    half := (num-ec + 1) / 2
    half.repeat: | i |
      j := num-ec - i
      tmp := gen[i]
  ...
  /** Builds the complete QR matrix with patterns, data, mask, and format info. */
  static build-matrix_ data-codewords/ByteArray ec-codewords/ByteArray -> List:
    ...
    // Place data bits and apply the best mask.
  ...
  /** Places a 7x7 finder pattern with its top-left corner at ($row, $col). */
  ...
  /** Places the white separators around the finder patterns. */
  ...
  /** Places a 5x5 alignment pattern centered at ($center-row, $center-col). */
  ...
  /** Places the timing patterns (alternating dark and light modules). */
  ...
  /** Reserves the format-information areas. */
  ...
  /**
  Places the data bits in the matrix following the QR zigzag pattern.

  Uses Nayuki's algorithm with a separate loop counter to correctly skip the
  ...
  */
  static place-data-bits_ matrix/List reserved/List codewords/ByteArray size/int:
    ...
    // The loop variable always decrements by 2. The actual column shifts left
    // by 1 when <= 6, producing the pairs:
  ...
  /** Tries all 8 masks, scores each, and applies the best one. */
  ...
  /** Applies the mask pattern $mask-idx to the data modules. */
  ...
  /**
  Writes the format information (ECC level L and mask $mask-idx) into the matrix.

  Uses BCH(15, 5) encoding with generator polynomial 0x537 and mask 0x5412.
  */
  static write-format-info_ matrix/List size/int mask-idx/int:
    ...
    // Second copy: around the bottom-left and top-right finders.
    7.repeat: | i |
      bit := (format-bits >> (14 - i)) & 1
  ...
  /** Scores a masked matrix using all four QR penalty rules. */
  static score-matrix_ matrix/List size/int -> int:
    ...
    // Penalty 2: 2x2 blocks of the same color.
    (size - 1).repeat: | row |
      (size - 1).repeat: | col |
        v := matrix[row][col]
...
The bitmap is MSB-first, with rows packed to byte boundaries. This format is
  suitable for the Lightbug e-ink display (`draw-bitmap`).

Use $from-matrix to build a bitmap from a $QrCode matrix.
*/
class QrBitmap:
  /** The 1-bit-per-pixel bitmap data, MSB-first with byte-aligned rows. */
  data/ByteArray
  ...
  /**
  Creates a bitmap from the given QR $matrix.

  Each QR module is rendered as a $scale by $scale block of pixels, surrounded
    by a quiet zone of $quiet modules.
  */
